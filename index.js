...
async function main() {
  ...
  const avgFixed = {
    download: argv.dw || 110.24 * 1000 * 1000,
    upload: argv.up || 60.13 * 1000 * 1000,
    latency: argv.lt || 19,
  }
  var log = [];
  ...
  for (i = 0; i < iterazioni; i++) {
    ...
    page.on('console', function(msg) { try { log.push(JSON.parse(msg.text())); } catch { /*console.log(msg.text());*/ } });
